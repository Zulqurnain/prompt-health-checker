...
import type { RewrittenPrompts } from "@/types";
import { CopyButton } from "./CopyButton";
import { clsx } from "clsx";

interface RewrittenPromptsProps {
...
export function RewrittenPromptsCard({ rewritten }: RewrittenPromptsProps) {
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-6">
      ...
      <div className="relative bg-slate-950/60 rounded-xl p-4 border border-slate-800/60">
        <pre className="text-sm text-slate-200 whitespace-pre-wrap break-words font-mono leading-relaxed pr-16">
          {content}
        </pre>
