    ...
    <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-6">
      ...
      <div className="flex flex-col gap-2">
        {history.map((item) => (
          <div
            key={item.id}
            className="flex items-start gap-3 bg-slate-800/40 border border-slate-700/40 rounded-xl p-3 hover:border-slate-600/50 transition-colors group"
          >
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400 leading-relaxed line-clamp-2">
                {item.prompt.slice(0, 150)}{item.prompt.length > 150 ? "…" : ""}
              </p>
            </div>
            <div className="flex flex-col gap-1 shrink-0">
              <button
                onClick={() => onSelect(item)}
                className="text-[10px] text-sky-500 hover:text-sky-300 transition-colors font-medium"
              >
                Load
              </button>
              <button
