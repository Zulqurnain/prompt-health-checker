...
function Section({
  ...
  title: string;
  items: string[];
  variant: "good" | "weak" | "ambiguity" | "missing" | "improve";
}) {
  const config = {
  ...
  return (
    <div className={clsx("rounded-xl border p-4", config.bg)}>
      <h3 className={clsx("text-xs font-semibold uppercase tracking-wider mb-3", config.color)}>
        {title}
      </h3>
      <ul className="flex flex-col gap-1.5">
        {items.map((item, i) => (
          <li key={i} className="flex gap-2 items-start text-sm text-slate-300">
            <span className={clsx("shrink-0 text-xs mt-0.5 w-3", config.color)}>{config.icon}</span>
...
export function AnalysisSummaryCard({ summary, costSavingsNote }: AnalysisSummaryProps) {
  return (
    <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-6">
      ...
      <div className="mt-4">
        <Section title="How to Improve" items={summary.howToImprove} variant="improve" />
      </div>
      ...
      <div className="mt-4 flex items-start gap-2.5 bg-violet-500/8 border border-violet-500/20 rounded-xl p-4">
        ...
        <div>
          <div className="text-xs font-semibold text-violet-400 mb-1 uppercase tracking-wider">Cost Efficiency</div>
          <p className="text-xs text-slate-300 leading-relaxed">{costSavingsNote}</p>
        </div>
      </div>
    </div>
  );
