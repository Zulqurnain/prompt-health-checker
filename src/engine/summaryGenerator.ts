import type { AnalysisSummary, StructureComponents, WeakWordMatch, AmbiguityIssue } from "@/types";
...
export function generateSummary(
  ...
  if (weakWords.length > 0)
    whatIsWeak.push(`${weakWords.length} hedge/weak word(s) add uncertainty (${[...new Set(weakWords.map(w => w.word.toLowerCase()))].slice(0, 5).join(", ")}).`);
  if (wordCount < 15) whatIsWeak.push("Prompt is too short to give the AI enough direction.");
  if (!structure.context && wordCount < 50)
    whatIsWeak.push("No background context — AI must make assumptions about your situation.");

  if (whatIsWeak.length === 0 && score < 60)
    whatIsWeak.push("Several structural prompt elements are missing.");

  // Ambiguity areas
  for (const issue of ambiguityIssues) {
    ambiguityAreas.push(issue.description);
  }
  if (ambiguityAreas.length === 0) {
    ambiguityAreas.push("No major ambiguity detected in this prompt.");
  }
  ...
  if (missingDetails.length === 0)
    missingDetails.push("All major structural components are present.");
  ...
  if (!structure.examples)
    howToImprove.push("Provide a short example of what good output looks like.");
  if (weakWords.length > 2)
    howToImprove.push("Replace hedge words with confident, direct language.");
