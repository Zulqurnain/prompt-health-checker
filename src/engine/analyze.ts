...
function buildCostSavingsNote(
  ...
  if (score < 40) {
    issues.push(
      "low-quality prompts often require 2–3 retry attempts, multiplying your token spend"
    );
  }
  if (weakWordsCount > 3) {
    issues.push(
      "hedge words signal uncertainty to the AI, often producing verbose, hedged responses that waste output tokens"
    );
  }
  if (!hasOutputFormat) {
    issues.push(
      "without a format constraint the AI may over-generate, using 2–4x more output tokens than needed"
    );
  }

  if (issues.length === 0) {
    return "This prompt is well-structured and should produce efficient, targeted responses with minimal token waste.";
  }
...
export function analyzePrompt(rawText: string): PromptAnalysisResult {
  ...
  const costSavingsNote = buildCostSavingsNote(
    score,
    weakWords.length,
