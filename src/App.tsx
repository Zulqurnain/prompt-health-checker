...
import { WeakWordsHighlight } from "@/components/WeakWordsHighlight";
import { TokenEstimates } from "@/components/TokenEstimates";
import { Suggestions } from "@/components/Suggestions";
import { RewrittenPromptsCard } from "@/components/RewrittenPrompts";
import { AnalysisSummaryCard } from "@/components/AnalysisSummary";
import { HistoryPanel } from "@/components/HistoryPanel";
import { analyzePrompt } from "@/engine/analyze";
import { saveToHistory, loadHistory, clearHistory } from "@/utils/localStorage";
import type { PromptAnalysisResult, PromptHistoryItem } from "@/types";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center shadow-lg">
              <svg viewBox="0 0 20 20" className="w-4 h-4 fill-white">
                <path d="M4 3h12a1 1 0 011 1v2H3V4a1 1 0 011-1zM3 8h14v8a1 1 0 01-1 1H4a1 1 0 01-1-1V8z"/>
              </svg>
            </div>
            <div>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-12 pb-8">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 max-w-xl mx-auto text-base leading-relaxed">
            Analyze prompt quality, estimate tokens across AI model families, and strengthen
            prompts before sending them to AI.
          </p>
        </div>
      ...
      {/* Results */}
      {result && (
        <section id="results" className="max-w-5xl mx-auto px-4 pb-12 animate-fade-in">
      ...
      <footer className="border-t border-slate-800/60 bg-slate-900/30">
        <div className="max-w-5xl mx-auto px-4 py-6 text-center">
          <p className="text-xs text-slate-600 leading-relaxed max-w-2xl mx-auto mb-2">
            <strong className="text-slate-500">Token estimates are approximate.</strong>{" "}
            Consumer AI apps (ChatGPT, Claude.ai, Gemini, Cursor, Grok) inject hidden system prompts, memory, tools,
            and agent context. True app-level token counts are not observable from outside the API. Counts shown here
